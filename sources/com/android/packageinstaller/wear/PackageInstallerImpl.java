...
public class PackageInstallerImpl {
    ...
    public void install(String str, ParcelFileDescriptor parcelFileDescriptor, InstallListener installListener) {
        try {
            PackageInstaller.SessionInfo sessionInfo = this.mSessionInfoMap.get(str);
            PackageInstaller.Session session = sessionInfo != null ? getSession(str) : null;
            if (session == null) {
                try {
                    innerCreateSession(str);
                    sessionInfo = this.mSessionInfoMap.get(str);
                    try {
                        session = this.mPackageInstaller.openSession(sessionInfo.getSessionId());
                        this.mOpenSessionMap.put(str, session);
            ...
            InstallTask installTask = new InstallTask(this.mContext, str, parcelFileDescriptor, installListener, session, getCommitCallback(str, sessionInfo2.getSessionId(), installListener));
            installTask.execute();
    ...
    private IntentSender getCommitCallback(final String str, final int i, final InstallListener installListener) {
        BroadcastReceiver broadcastReceiver = new BroadcastReceiver() { // from class: com.android.packageinstaller.wear.PackageInstallerImpl.1
    ...
    public void handleCommitCallback(Intent intent, String str, int i, InstallListener installListener) {
